...
export default function JourneyTimeline() {
  ...
  return (
    ...
    >
      {/* Subtle blue glow */}
      <div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[300px] blur-[140px] opacity-[0.04] pointer-events-none"
...
<div className="flex items-center gap-[9px] mb-4">
              ...
              >
                ...
                >
                  <div
                    ...
                    style={{ background: step.accent }}
                  />
                  <h3 className="font-bold text-[0.92rem] text-[#111]">
                    {step.title}
                  </h3>
                  <p className="text-[0.72rem] text-black/45 leading-[1.75]">
                    {step.desc}
                  </p>
                </div>
              </div>
      ...
      <div className="lg:hidden flex flex-col gap-6 relative">
        {/* Vertical line */}
        <div
          className="absolute left-[27px] top-0 bottom-0 w-[1.5px]"
          style={{
            background:
              "linear-gradient(to bottom, rgba(26,86,219,0.2), transparent)",
          }}
        />
        ...
        {steps.map((step, i) => {
          ...
          return (
            <div key={step.title} className="flex gap-5 items-start">
              {/* Icon */}
              <div
                className="w-14 h-14 rounded-[16px] flex items-center justify-center shrink-0 relative z-10"
                style={{
                  background: `linear-gradient(145deg, #ffffff, #f5f5f0)`,
                  border: `1px solid ${step.accent}25`,
              ...
              >
                ...
                />
                <div
                  className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full flex items-center justify-center text-[0.5rem] font-black text-white"
                  style={{ background: step.accent }}
                >
                  {i + 1}
                </div>
              ...
              <div className="flex-1 pt-1">
                <h3 className="font-bold text-[0.95rem] text-[#111] mb-2">
                  {step.title}
                </h3>
                <p className="text-[0.78rem] text-black/45 leading-[1.75]">
                  {step.desc}
                </p>
              </div>
            </div>

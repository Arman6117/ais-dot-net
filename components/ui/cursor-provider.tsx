...
export default function CursorProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const xToGlow = gsap.quickTo(glowRef.current, "x", { duration: 1.5, ease: "power2.out" });
    const yToGlow = gsap.quickTo(glowRef.current, "y", { duration: 1.5, ease: "power2.out" });
  ...
  <div 
        ...
        style={{ 
          background: 'radial-gradient(circle, rgba(26, 86, 219, 0.2) 0%, transparent 0%)',
          filter: 'blur(80px)'
        }}
      />
      <div
